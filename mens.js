var mensData = [
  ...
  let cartArr = JSON.parse(localStorage.getItem("fromMens")) || []
  function displayData(mensData){
      mensData.forEach(function(el){
        ...
        Pprice.innerText ="$"+ el.strikedoffprice
        
        box1.append(Sprice,Pprice)
        ...
        box.append(img,brand,Pname,box1,btn)
        document.querySelector("#container").append(box)
    })
    
  }
     function cartFunc(el){
      cartArr.push(el)
      localStorage.setItem("fromMens",JSON.stringify(cartArr))
     if(confirm("Go To CartPage")){
         window.location.href="/Cart.html"
     }
    }
  let proceed = document.querySelector("#proceedToBag")
  ...
  let filter = document.querySelector("#sortName")
  filter.addEventListener("change",function(event){
    event.preventDefault()
    // console.log("inside filter2")
    if(filter.value=="All"){
  ...
  let priceFilter = document.querySelector("#sortPrice")
...
let nameFilter = document.querySelector("#sortByName")
nameFilter.addEventListener("change",nameSortFun)

function nameSortFun(){
  event.preventDefault()
  document.querySelector("#container").innerHTML = ""
  if(nameFilter.value == "All"){
     return displayData(mensData)
  }else if(nameFilter.value=="ASC"){
      mensData.sort(function(a,b){
        if(a.name>b.name)
        return 1 ; 
